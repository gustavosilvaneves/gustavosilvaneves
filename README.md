🌟 Bem-vindo ao meu perfil! Eu sou o Gustavo 👋
<p align="center"> <img src="[https://i.imgur.com/bkYH1yH.png](https://imgur.com/a/zkPtuFo)" width="100%" alt="Banner" /> </p>
👨‍💻 Sobre mim

Olá! Meu nome é Gustavo, e atuo profissionalmente como desenvolvedor Full Stack.
Trabalho diariamente com tecnologias modernas, construindo aplicações completas — do front-end ao back-end — com foco principal em TypeScript, minha linguagem mais utilizada.

Sou apaixonado por tecnologia, performance, boas práticas e por transformar ideias em soluções reais.

🚀 Tecnologias que utilizo
<p align="left"> <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="50" height="50" alt="TypeScript" /> <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="50" height="50" alt="React" /> <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="50" height="50" alt="JavaScript" /> <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="50" height="50" alt="HTML" /> <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="50" height="50" alt="CSS" /> </p>
📊 Minhas estatísticas
<p align="center"> <img height="180" src="https://github-readme-stats.vercel.app/api?username=gustavosilvaneves&show_icons=true&theme=tokyonight" /> <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=gustavosilvaneves&layout=compact&theme=tokyonight" /> </p>
📌 Principais projetos

Aqui estão alguns destaques do que tenho construído:

🚀 Aplicações web modernas usando React + TypeScript

🧠 APIs completas com Node.js e boas práticas

⚡ Integração com serviços externos

🛠️ Ferramentas e automações para melhorar produtividade

🎨 Interfaces limpas e responsivas

(Se quiser, posso adicionar cards automáticos com seus melhores repositórios.)

🌐 Conecte-se comigo
<p align="left"> <a href="https://wa.me/5577999162512" target="_blank"> <img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" /> </a> <a href="https://instagram.com/digital.gusttavo" target="_blank"> <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" /> </a> </p>
✨ Sempre evoluindo

Sou movido pelo desafio de aprender, melhorar e criar algo maior a cada novo projeto.
Se quiser trocar ideias, colaborar ou conversar sobre tecnologia, estarei sempre aberto!
